...
# Define variables
u = opti.variable(2)         # Control input [u_x, u_y]
...
V = 0.5 * ca.dot(p_sym - p_goal_sym, p_sym - p_goal_sym) + 0.5 * ca.dot(u, u)
dV_dp = p_sym - p_goal_sym + u

# Define QP cost: 0.5 * ||u - u_des||^2 + slack_weight * s^2
cost = 0.5 * ca.dot(u - u_des_sym, u - u_des_sym) + slack_weight_sym * s**2 + slack_weight_sym * sc**2

# Define constraints
cbf_constraint = ca.dot(dh_dp, u) >= -alpha_sym * h - sc  # CBF with slack
clf_constraint = ca.dot(dV_dp, u) <= -gamma_sym * V + s # CLF
...
plt.grid(True)
plt.axis('equal')
plt.savefig('obstacle_avoidance_sqrt_cbf.png', bbox_inches='tight', dpi=100)
plt.close()
print(f"Simulation completed. Plot saved as 'obstacle_avoidance_sqrt_cbf.png'.")
